...
<div class="ml15 mr15 div-open">
    ...
    <div class="mt10 animated fadeInDown">
        <div class="inlininput mr10">
            ...
            <input type="text" id="sd" class="w100" value="2015/07/23 08"
                   onClick="WdatePicker({dateFmt:'yyyy/MM/dd HH',maxDate:'#F{$dp.$D(\'ed\')}'})"/>
        </div>
        <div class="inlininput mr10">
            <label class="S-label">结束时间</label>
            <span class="add-on"><a href="#"><i class="icon-calendar"></i></a></span>
            <input type="text" id="ed" class="w100" value="2015/07/23 18"
        ...
        <a href="#" class="btn btn-primary btn-sbox vm mb10">
            定界流程
        </a>
    </div>
    <!-- 搜索-e -->
        ...
        <div class="userinfobox clearfix">
            ...
            <div class="pull-left pr">
                ...
                <div class="user-infotext">
                    <h1 id="info.msisdn">&nbsp;</h1>

                    <p class="" id="info.tac">&nbsp;</p>

                    <p class="mt5"><i class="uicon ui-Signal mr10"></i><span class="text-b1bcc5 vm" id="info.rat">&nbsp;
            </span></p>
                </div>
            </div>
            <!-- 用户信息头像-e -->
            ...
            <div class="pull-right">
                <ul class="userinfo-datalist clearfix">
                    <li>
                        <a href="#">
                            <h2 class="pr" id="detcnt">0<i class="pa">详单</i></h2>
    ...
    <div class="col-2-r320-2 fl animated fadeInRight">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <i class="uicon ui-paneltitel mr10"></i>
                高流量风险业务<i class="text-danger" id="ywcnt">（3）</i>
            </div>
...
<script type="text/javascript">
    ...
    function inittimeLine(ele) {
        ele.html("");
        var sd = Number($("#sd").val().substr(-2, 2));
        var ed = Number($("#ed").val().substr(-2, 2));
        var opstr =
                "<div class='any-title'> <table class='anydata-table'> <tbody> <tr> <th> <div class='timebox'></div>" +
                "<i class='time-dotline'></i> </th> </tr> </tbody> </table> </div> <div class='any-data'>" +
                "<table class='anydata-table'> <tbody><tr>";
        var edstr = "</tr></tbody> </table> </div>";
        var str = "";
        for (var i = sd; i <= ed; i++) {
            str += "<th> <div class='timebox'><p >" + i + ":00</p> </div> <i class='time-dotline'></i> </th>";
        }
...
//            rowno += " <label class='text_overflow'>" + data[i].name + "</label>";
            rowdata +=
                    "<div class='data-line'> <table class='anydata-table table-set'> <tbody><tr><td class='w100' >"+data[i].name+"</td>";
            for (var j = sd; j <= ed; j++) {
                var isT = false;
                ...
                rowdata += "<td class='tdheight'><i class='data-tip " + (isT ? "stoptip" : "") + "'></i></td>";

            }
            rowdata += " </tr> </tbody> </table> </div>";

        }
//        rowno = "<div class='any-title'>" + rowno + "</div>";


        var datahtml = "<div >" + rowdata + "</div>";
    ...
    $("#psn").click(function (e) {
        showMask();
        inittimeLine($("#b1 > .pr.mt5"));
        inittimeLine($("#b2 > .pr.mt5"));
        inittimeLine($("#b3 > .pr.mt5"));
        inittimeLine($("#b4 > .pr.mt5"));
        inittimeLine($("#b2-1 > .pr.mt5"));
        inittimeLine($("#b2-2 > .pr.mt5"));
    ...
    function buildlv2html(d, div, lv1name) {
        ...
        for (var i = 0; i < d.length; i++) {
            ...
            contentdiv += "<div class='tab-pane " + (i == 0 ? "active" : "") + "' id='Prodeb" + d[i].id + "'>" +
                    ...
                    "</p> <p class='rescon'> <label class='mb5'>问题类型：</label> <span class='btn btn-danger btn-small mb5 mr5'>" + d[i].desc + "</span> </p>" +
                    "</div> <div> <h5 class='restitle'>解释口径</h5> <p class='Explainpg'>" +
                    d[i].ex + "</p> </div> </div> </div> </div>";
        }
        var ed1 =
